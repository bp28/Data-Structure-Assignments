...
void binary_heap::insert(huffman_node* hn) {
...
void binary_heap::percolateUp(int hole) {
    ...
    int x = (node -> getFrequency());
    // while we haven't run off the top and while the
    // value is less than the parent...
    for ( ; (hole > 1) && (x < (heap[hole/2] -> getFrequency())); hole /= 2 )
        heap[hole] = heap[hole/2]; // move the parent down
    // correct position found!  insert at that spot
    heap[hole] = node;
}

huffman_node* binary_heap::deleteMin() {
...
void binary_heap::percolateDown(int hole) {
    ...
    while ( hole*2 <= heap_size ) {
        ...
        if ( (child+1 <= heap_size) && (heap[child+1] -> getFrequency()) < (heap[child] -> getFrequency()) )
            child++;
        // if the child is greater than the node...
        if ( x > heap[child] -> getFrequency() ) {
